    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) || Input.GetMouseButtonDown(1))
        {
            ...
            if (Physics.Raycast(ray, out hit, editDistance))
            {
                ...
                Vector3 posAdd = add ? hit.point + (hit.normal * 0.5f) : hit.point + (hit.normal * -0.5f);
                edit(new Vector3Int(Mathf.RoundToInt(posAdd.x - 1), Mathf.RoundToInt(posAdd.y), Mathf.RoundToInt(posAdd.z - 1)));
